...
// RUN: %clang_cc1 -std=c++17 -verify -Wno-vla -fopenmp-simd -x c++ -fblocks -DBLOCKS -triple x86_64-apple-darwin10 -emit-llvm %s -o - | FileCheck %s --implicit-check-not="{{__kmpc|__tgt}}"
// RUN: %clang_cc1 -std=c++20 -DDIAGOK -verify=expected,cpp20 -Wno-vla -fopenmp-simd -x c++ -fblocks -DBLOCKS -triple x86_64-apple-darwin10 -emit-llvm %s -o - | FileCheck %s --implicit-check-not="{{__kmpc|__tgt}}"
...
#elif defined(BLOCKS)
  ^{
#pragma omp parallel master taskloop lastprivate(g, sivar) // cpp20-warning{{use of result of assignment to object of volatile-qualified type}}
  for (int i = 0; i < 10; ++i) {
